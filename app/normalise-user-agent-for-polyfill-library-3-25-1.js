export function useragent_parser(ua) {
  ...
  if (!ua) {
    ...
    (result = /(ESPN)[ %02|]+Radio\/(\d+)\.(\d+)\.(\d+) CFNetwork/.exec(ua))
  ) {
    let family = result[1];
    let major = result[2];
    let minor = result[3];
    ...
    (result =
      /(?:\/[\d.A-Za-z]+|) {0,5}([\w !:[\]-]{0,50}(?:[Aa]rchiver|[Ii]ndexer|[Ss]craper|[Bb]ot|[Ss]pider|[Cc]rawl[a-z]{0,50}))[ /](\d+)(?:\.(\d+)(?:\.(\d+)|)|)/.exec(
        ua
      ))
  ) {
    let family = result[1];
    ...
    (result =
      /((?:[A-Za-z][\d A-Za-z-]{0,50}|)[^C][^Uu][Bb]ot)\b(?:(?:[ /]| v)(\d+)(?:\.(\d+)|)(?:\.(\d+)|)|)/.exec(
        ua
      ))
  ) {
    let family = result[1];
    ...
    (result =
      /((?:[\dA-z]{1,50}|[A-z-]{1,50} ?|)(?: the |)(?:[Ss][Pp][Ii][Dd][Ee][Rr]|[Ss]crape|[Cc][Rr][Aa][Ww][Ll])[\dA-z]{0,50})(?:(?:[ /]| v)(\d+)(?:\.(\d+)|)(?:\.(\d+)|)|)/.exec(
        ua
      ))
  ) {
    let family = result[1];
    ...
    let family = "NetFront NX";
    return {family,major,minor,patch};
  } else if ((result = /(Silk)\/(\d+)\.(\d+)(?:\.([\d-]+)|)/.exec(ua))) {
    ...
    (result =
      /^(HTC) Streaming Player(?: \S+ \/){3} (\d+)\.(\d+)(?:\.(\d+)|)/.exec(
        ua
      ))
  ) {
    let family = result[1];
    ...
    (result = /(Java)[ /]{0,1}\d+\.(\d+)\.(\d+)[_-]*([\dA-Za-z]+|)/.exec(ua))
  ) {
    let family = result[1];
    let major = result[2];
    let minor = result[3];
