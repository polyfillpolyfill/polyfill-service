...
export function UA(uaString) {
	...
	uaString = uaString.slice(0, Math.max(0, MAX_UA_LEN));
	...
	if (normalized) {
		this.ua = new Agent(normalized[1], normalized[2], (normalized[3] || 0), (normalized[4] || 0));
	} else {
		...
		if (aliases[this.ua.family]) {
			...
			// Map to different family with constant version
				...
				this.ua = new Agent(a[0], a[1], (a[2] || 0), (a[3] || 0));
...
UA.prototype.isUnknown = function() {
	return (!Object.keys(baseLineVersions).includes(this.ua.family)) || !this.meetsBaseline();
};

UA.normalize = function(uaString) {
	if (/^\w+\/\d+(\.\d+(\.\d+)?)?$/i.test(uaString)) {
