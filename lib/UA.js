	...
	if (normalized) {
		...
		if (aliases[this.ua.family]) {
			...
			// Map to different family with per-version mapping
				...
				for (const semverExpr in aliases[this.ua.family]) {
...
UA.normalizeName = function(uaName, version) {
	const name = aliases[uaName.toLowerCase()] || uaName.toLowerCase();
	if (typeof name === 'string') return name;
	if (Array.isArray(name)) return name[0];
	if (typeof name === 'object' && version) {
		const ver = version.split('.');
		const ua = new useragent.Agent(uaName, ver[0], (ver[1] || 0), (ver[2] || 0));
		for (const semverExpr in name) {
			if (ua.satisfies(semverExpr)) return name[semverExpr][0];
		}
