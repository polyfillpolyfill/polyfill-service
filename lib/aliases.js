...
var AliasResolver = function(resolvers) {
	...
	/** An array of functions that map a feature name and properties identifier (e.g. an alias)
...
AliasResolver.prototype.resolve = function(featureSet) {
	return this.polyfillNameResolverFunctions.reduce(function(resolvingSetPromise, resolverFunction) {
...
function applyResolverToIdentifiers(featureSet, nameResolverFunction) {
	...
	return Promise.all(Object.keys(featureSet).map(function(featureName) {
		...
		return Promise.resolve(nameResolverFunction(featureName)).then(function(resultItems) {
			...
			// Alias resolver does not know this feature - do not modify feature set
			...
			// Alias resolver returned a list that did not include the original name, so remove that from the feature set
