...
function applyResolverToIdentifiers(featureSet, nameResolverFunction) {
	let newNamesDiscovered = false;

	function run() {
		return Promise.all(Object.keys(featureSet).map(featureName => {
			const feature = featureSet[featureName];
			return Promise.resolve(nameResolverFunction(featureName)).then(resultItems => {
