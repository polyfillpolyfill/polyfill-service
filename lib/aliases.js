...
function applyResolverToIdentifiers(featureSet, nameResolverFunction) {
	...
	let invokeCount = 0;
	console.log('Start', nameResolverFunction.name);
	console.time(nameResolverFunction.name, processQueue.length);
		...
		resultItems.forEach(function(newName) {
			...
			if (newName !== featureName) {
				const result = Object.assign({}, feature, {
					aliasOf: uniq([featureName].concat(feature.aliasOf || [])),
					flags: feature.flags || []
				});
				...
				if (!featureSet[newName]) {
					...
					['flags', 'aliasOf'].forEach(prop => {
						featureSet[newName][prop] = (prop in featureSet[newName]) ? uniq(featureSet[newName][prop].concat(result[prop])).sort() : result[prop];
					});
				}
			}
		});
	...
	console.timeEnd(nameResolverFunction.name);
	console.log("Invoke count", nameResolverFunction.name, invokeCount);
