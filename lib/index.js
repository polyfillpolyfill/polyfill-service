...
const streamToString = require('stream-string');
	...
	return Promise.resolve()
		...
		.then(features => {
			...
			return Promise.all(Object.keys(features).map(featureName => {
				...
				return sourceslib.getPolyfill(featureName).then(polyfill => {
					if (!polyfill) {
						...
						feature.commonjs = (polyfill.build && polyfill.build.commonjs);
			...
			.then(() => {
				...
				let commonjsModuleInBundle = false;
				...
				graph.sort().forEach(featureName => {
					...
					if (features[featureName].commonjs) {
						commonjsModuleInBundle = true;
					}
				});

				if (commonjsModuleInBundle) {
					const browserifyBundle = browserify(intoStream(builtPolyfillString),
						{
							basedir: path.resolve(__dirname, '../polyfills')
						});
					if (options.minify) {
						browserifyBundle.transform({
							global: true
						}, 'uglifyify');
					}
					return streamToString(browserifyBundle.bundle());
				} else {
					return builtPolyfillString;
				}
			})
			.then(function(builtPolyfillString) {
				if (builtPolyfillString) {
					let rum = '';
					if (options.rum) {
						rum = rumTemplate({baseurl: options.baseurl, features:features});
					}
