	...
	return Promise.resolve()
		.then(() => {
			...
			if ((!ua.meetsBaseline() || ua.isUnknown()) && options.unknown !== 'polyfill') {
				...
				const optionsForAll = Object.assign({}, options);
				optionsForAll.features = {};
				Object.keys(options.features).forEach(featureName => {
					optionsForAll.features[featureName] = {flags: ['always']};
				});
		...
		.then(featureLists => {
			...
			.then(() => {
				...
				if (builtPolyfillString) {
					let rum = '';
					if (options.rum && process.env.RUM_MYSQL_DSN) {
						rum = rumTemplate({baseurl: options.baseurl, features:allFeatures});
					}
