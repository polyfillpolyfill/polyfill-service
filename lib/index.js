...
function getPolyfillString(options) {
	...
	let outputString = options.stream ? new ReadableStream : [];
			...
			.then(() => {
				...
				if (!options.minify) {
					...
					if (!ua.meetsBaseline() && ua.getBaseline()) {
						outputString.push('Version range for polyfill support in ' + ua.getFamily() + ' is: ' + ua.getBaseline() + '\n\n');
					}
					sortedFeatures.forEach(featureName => {
						if (allFeatures[featureName] && allFeatures[featureName].comment) {
							outputString.push(allFeatures[featureName].comment);
						}
					});
					if (warnings.unknown.length) {
						outputString.push('\nThese features were not recognised:\n' + warnings.unknown.join(''));
					}
					...
					outputString.push('/* Disable minification (remove `.min` from URL path) for more info */\n');
				}
