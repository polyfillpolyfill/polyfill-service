...
function getPolyfillString(options) {
	...
	.then(targetedFeatures => {
		...
		if (!sortedFeatures.length) {
			if (!options.minify) {
				output.add(streamFromString('/* No polyfills found for current settings */\n'));
			}
			return;
		}
		...
		sortedFeatures.forEach(featureName => {
			...
			output.add(new lazystream.Readable(function () {
	...
	return options.stream ? output : streamToPromise(output).then(bufs => {
