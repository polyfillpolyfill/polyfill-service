...
function getPolyfillString(options) {
	...
	const supportedUA = ((!ua.meetsBaseline() || ua.isUnknown()) && options.unknown !== 'polyfill');
	const taskPromise = supportedUA ? Promise.resolve({}) : getPolyfills(options)
	...
	return options.stream ? output : new Promise(resolve => {
		taskPromise.then(strm => {
			const bufs = [];
			strm.on('data', chunk => bufs.push(chunk));
			strm.on('end', () => resolve(Buffer.concat(bufs).toString('UTF-8')));
		});
	});
}
