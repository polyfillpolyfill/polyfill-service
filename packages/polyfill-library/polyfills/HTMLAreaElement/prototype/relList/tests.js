...
describe('HTMLAreaElement.prototype.relList', function() {
	...
	if (hasGetOwnPropertyDescriptor) {
		it('has a getter function', function(){
			var descriptor = Object.getOwnPropertyDescriptor(HTMLAreaElement.prototype, 'relList');
	...
	describe('(setter)', function() {
		it('updates the element\'s corresponding attribute', function(){
			...
			it('should update the DOMTokenList instance with the new length and tokens', function() {
				var area = document.createElement('area');
				area.relList = 'a a b';
				proclaim.equal(area.relList.length, 2);
				proclaim.isTrue(area.relList.contains('a'));
				proclaim.isTrue(area.relList.contains('b'));
			});
		});
	});
});