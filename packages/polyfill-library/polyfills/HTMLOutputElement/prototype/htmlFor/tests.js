...
describe('HTMLOutputElement.prototype.classList', function() {
	...
	if (hasGetOwnPropertyDescriptor) {
		it('has a getter function', function(){
			var descriptor = Object.getOwnPropertyDescriptor(HTMLOutputElement.prototype, 'classList');
	...
	describe('(setter)', function() {
		it('updates the element\'s corresponding attribute', function(){
			...
			it('should update the DOMTokenList instance with the new length and tokens', function() {
				var output = document.createElement('output');
				output.classList = 'a a b';
				proclaim.equal(output.classList.length, 2);
				proclaim.isTrue(output.classList.contains('a'));
				proclaim.isTrue(output.classList.contains('b'));
			});
		});
	});
});