...
(function (global) {
	...
	function polyfillElement(element) {
		...
		if (/^(input|textarea)$/i.test(element.nodeName) && /^(email|number|password|search|tel|text|url|)$/i.test(element.getAttribute('type'))) {
			var
			xInput = document.createElement('-ms-input'),
			xPlaceholder = xInput.appendChild(document.createElement('-ms-placeholder')),
			xPlaceholderText = xPlaceholder.appendChild(document.createTextNode(element.placeholder)),
			...
			element.attachEvent('onpropertychange', function (event) {
				if (event.propertyName == 'placeholder') {
					xPlaceholder.innerHTML = element.placeholder;
				}
			});
		}
