	...
	if (!ua) {
		...
		patch = result[4];
		return { family, major, minor, patch };
	} else if (result = /^(.*)\/(\d+)(?:\.(\d+)|)(?:\.(\d+)|)(?:\.(\d+)|) CFNetwork/.exec(
		ua
		)){
		const family = result[1];
		const major = result[2];
		...
		patch = result[4];
		return {family,major,minor,patch};
	} else if (result = /Mozilla.*Mobile.*(Flipboard-Briefing).(\d+)\.(\d+)\.(\d+)/.exec(
		ua
		)){
		const family = result[1];
		const major = result[2];
		...
		patch = result[4];
		return {family,major,minor,patch};
	} else if (result = /(MozillaDeveloperPreview)\/(\d+)\.(\d+)([ab]\d+[a-z]*|)/.exec(
		ua
		)){
		const family = result[1];
		const major = result[2];
		...
		patch = result[4];
		return {family,major,minor,patch};
	} else if (result = /(Opera Mini)(?:\/att|)\/?(\d+|)(?:\.(\d+)|)(?:\.(\d+)|)/.exec(
		ua
		)){
		const family = result[1];
		const major = result[2];
		...
		patch = result[4];
		return {family,major,minor,patch};
	} else if (result = /\b(Dolphin)(?: |HDCN\/|\/INT\-)(\d+)\.(\d+)(?:\.(\d+)|)/.exec(
		ua
		)){
		const family = result[1];
		const major = result[2];
		...
		patch = result[4];
		return {family,major,minor,patch};
	} else if (result = /(Chromium|Chrome)\/(\d+)\.(\d+)(?:\.(\d+)|)(?:\.(\d+)|)/.exec(
		ua
		)){
		const family = result[1];
		const major = result[2];
		...
		patch = result[4];
		return {family,major,minor,patch};
	} else if (result = /(OC)\/(\d+)\.(\d+)\.(\d+)\.(\d+) \(Skype for Business\)/.exec(
		ua
		)){
		const family = "Skype";
		const major = result[2];
		...
		patch = result[4];
		return {family,major,minor,patch};
	} else if (result = /^(ViaFree|Viafree)-(?:tvOS-)?[A-Z]{2}\/(\d+)\.(\d+)\.(\d+)/.exec(
		ua
		)){
